...
public class Heavy {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N; i++) {
            heavy[i][0] = sc.nextInt();
            heavy[i][1] = sc.nextInt();
        }
        ...
        for (int i = 0; i < heavy.length; i++) {
            ...
            for (int j = 0; j < heavy.length; j++) {
                if (heavy[i][0] < heavy[j][0] && heavy[i][1] < heavy[j][1]) {
                    tempRank++;
                }
            }
        ...
        System.out.println(rank.stream().map(String::valueOf).collect(Collectors.joining(" ")));
